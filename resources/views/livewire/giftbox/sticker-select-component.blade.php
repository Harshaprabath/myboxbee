    ...
    <div class="builder-content-inner-body">
        ...
        <div class="container px-0 px-md-2">
            <div class="row no-gutters">
                ...
                <div class="col-6 col-md-3 px-2 px-lg-4" id="gift_sticker_{{$sticker->id}}"
                    onclick="$('#gift_sticker_modal_{{ $sticker->id }}').modal('show');">
                    @php
                    $stickerStatus = $cartItems->contains(function($item, $key) use ($sticker) {
                    return $item->id == $sticker->id && $item->options->status2 == 'sticker';
                    });
                    @endphp
                    <div class="builder-item-wrapper {{$stickerStatus?'active':''}}">
                        <div class="image position-relative">
                            <div>
                                <img class="w-100" src="{{ asset('upload') }}/{{ $sticker->image }} "
                                    alt="{{$sticker->name}}">
                            </div>
                            <div class="item-select-btn">
                                <div class="text1">{{$stickerStatus?'Selected':'Select'}}
                                </div>
                            </div>
                        </div>
                    ...
                    {{-- box modal --}}
                    <div class="modal fade" id="gift_sticker_modal_{{ $sticker->id }}">
                                                                ...
                                                                {{-- <div class="flexslider"
                                                                    ...
                                                                    {{-- <ul class="slides"> --}}
                                                                        {{-- <li
                                                                            ...
                                                                            <img src="{{ asset('upload') }}/{{$sticker->image}}"
                                                                                class="box_img_zoom" />
                                                                            {{--
                                                                        </li> --}}
                                                                        {{-- </ul> --}}
                                                    ...
                                                    <div class="button">
                                                        <div class="mb-2">
                                                            <button class="btn btn-block  rounded-0 border-0 py-2 px-4"
                                                                type="button"
                                                                wire:click.prevent="addStickerToCart({{$sticker->id}})"
                                                                wire:loading.attr="disabled"
                                                                @if($stickerStatus) disabled @endif
                                                                onclick="$('#gift_sticker_modal_{{ $sticker->id }}').modal('hide');"
